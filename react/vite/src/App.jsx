...
import {
  BASE, HERE, HOMEPAGE_URL, LOGIN_URL, LOGOUT_URL,
  API_BASE, DEFAULT_MODEL, TENANT_ID
} from "./config.js";
...
export default function NafsPremiumApp() {
  ...
  async function sendChat() {
    ...
    try {
      ...
      setChatText("");   // clear input setelah eksekusi
    }
  }
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-[#2a0f4a] via-[#1a0b2e] to-[#0c0516] text-white">
      {/* overlay pembeku saat belum login */}
      {!isLoggedIn && (
        <div className="fixed inset-0 z-40" aria-hidden="true" style={{ background: "transparent" }} />
      )}
            ...
            {active === "Chat" && (
              <div className="border-t border-white/10 p-3">
                <div className="grid gap-3">
                  ...
                  {/* Input + tombol (tanpa dropdown model) */}
                  <div className="flex flex-col lg:flex-row items-stretch gap-3">
                    <div className="flex-1">
                      <GhostInput
                        as="textarea"      // textarea: bisa enter baris baru
                        rows={4}
...
// opsional: supaya tetap kompatibel kalau suatu saat dipakai via window.* (Babel CDN)
