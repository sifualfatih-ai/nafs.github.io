...
(function (global) {
  // >>>>> PENTING: base URL otomatis <<<<<
  // Kalau halaman TIDAK berada di hostingersite.com / nafsflow.com,
  // request akan diarahkan ke domain Hostinger kamu.
  const API_BASE =
    (location.hostname.endsWith('hostingersite.com') || location.hostname.endsWith('nafsflow.com'))
      ? ''  // sudah di Hostinger → same-origin
      : 'https://linen-chamois-717264.hostingersite.com'; // GANTI jika domain Hostinger kamu beda

  // UI kecil (loading + toast)
  const NafsUI = {
    ...
    ensureOverlay() {
      ...
      el.innerHTML = `
        <div style="min-width:240px;background:#fff;color:#111;border-radius:14px;padding:16px 18px;box-shadow:0 10px 30px rgba(0,0,0,.35);font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Arial;text-align:center;">
          <span class="sp" style="width:18px;height:18px;border:2px solid #e2e8f0;border-top-color:#16a34a;border-radius:50%;display:inline-block;vertical-align:middle;margin-right:8px;animation:spin .9s linear infinite"></span>
    ...
    toast(message,type='info',ms=3000){
      ...
      el.textContent=message; this._toastWrap.appendChild(el); setTimeout(()=>el.remove(),ms);
    }
  };
