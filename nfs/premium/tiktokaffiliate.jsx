...
(function () {
  ...
  function TiktokAffiliatePage({ isLoggedIn, loginUrl, Field, GhostInput, ModelSelect }) {
    const vibeTags = [
    ...
    const [selectedVibes, setVibes]   = React.useState(new Set([vibeTags[0]]));
    const [ratio, setRatio]           = React.useState("9:16");
    const [prodFile, setProdFile]     = React.useState(null);
    const [prodPreview, setPreview]   = React.useState(null);
    const [cards, setCards]           = React.useState(Array.from({length:6},()=>({posterUrl:null,videoUrl:null,loading:false})));
    const [generatingGrid, setGen]    = React.useState(false);
    const fileInputRef                = React.useRef(null);

    const onPickFile = () => { if(!isLoggedIn) return; fileInputRef.current?.click(); };
    const onFileChange = (e) => {
      const f = e.target.files?.[0]; if(!f) return;
      setProdFile(f); setPreview(URL.createObjectURL(f));
    };
    ...
    async function makeMockPoster(title, model, vibe, ratioStr){
      const [w,h]=[900,1600];
      const c=document.createElement("canvas"); c.width=w; c.height=h;
      const ctx=c.getContext("2d");
      const g=ctx.createLinearGradient(0,0,w,h); g.addColorStop(0,"#2a0f4a"); g.addColorStop(1,"#1a0b2e");
      ctx.fillStyle=g; ctx.fillRect(0,0,w,h);
      ctx.fillStyle="#37415188"; ctx.fillRect(32,32,w-64,h-164);
      ctx.fillStyle="rgba(255,255,255,.85)"; ctx.font="24px Sans-Serif"; ctx.textAlign="center";
      ctx.fillText(title, w/2, h-120);
      ctx.fillStyle="rgba(255,255,255,.6)"; ctx.font="16px Sans-Serif";
      ctx.fillText(`Model: ${model}`, w/2, h-90); ctx.fillText(`Rasio ${ratioStr}`, w/2, h-66);
    ...
    async function handleGenerateVideo(idx){
      ...
      try{
        ...
        // panggil backend kamu (Railway / Hostinger)
          ...
          <div className="rounded-xl bg-white/[0.02] border border-white/10 p-3 space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {["Wanita","Pria","Kustom"].map(m=>(
                  <button key={m} onClick={()=>isLoggedIn && setModelType(m)} disabled={!isLoggedIn}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 max-h-[260px] overflow-auto pr-1">
                {vibeTags.map(tag=>(
                  <button key={tag} onClick={()=>toggleVibe(tag)} disabled={!isLoggedIn}
            ...
            <div>
              {isLoggedIn ? (
                <button onClick={(e)=>{e.preventDefault(); generateGrid();}} disabled={generatingGrid}
              ...
              {cards.map((card, idx)=>(
                <div key={idx} className="rounded-2xl bg-[#2a0f4a]/40 border border-white/10 overflow-hidden flex flex-col">
                  ...
                  <div className="p-2 border-t border-white/10">
                    {isLoggedIn ? (
                      <button onClick={()=>handleGenerateVideo(idx)} disabled={card.loading}
  ...
  // daftar halaman → kunci nama key harus PERSIS dengan data-page
